package com.phoenix.consumer.rabbitmq;
...
public class DlxProcessingErrorHandler {
  ...
  /** Dead exchange name */
  @NonNull private String deadExchangeName;

  private int maxRetryCount = 3;

  /**
   * Constructor. Will retry for n times (default is 3) and on the next retry will consider message
   * as dead, put it on dead exchange with given <code>dlxExchangeName</code> and <code>routingKey
   * </code>
   *
   * @param deadExchangeName dead exchange name. Not a dlx for work queue, but exchange name for
   *     really dead message (wont processed antmore).
   * @throws IllegalArgumentException if <code>dlxExchangeName</code> or <code>dlxRoutingKey</code>
   *     is null or empty.
   */
  ...
  /**
   * Constructor. Will retry for <code>maxRetryCount</code> times and on the next retry will
   * consider message as dead, put it on dead exchange with given <code>dlxExchangeName</code> and
   * <code>routingKey</code>
   *
   * @param deadExchangeName dead exchange name. Not a dlx for work queue, but exchange name for
   *     really dead message (wont processed antmore).
   * @param maxRetryCount number of retry before message considered as dead (0 >= <code>
   *      maxRetryCount</code> >= 1000). If set less than 0, will always retry
   * @throws IllegalArgumentException if <code>dlxExchangeName</code> or <code>dlxRoutingKey</code>
   *     is null or empty.
   */
  ...
  /**
   * Handle AMQP message consume error. This default implementation will put message to dead letter
   * exchange for <code>maxRetryCount</code> times, thus two variables are required when creating
   * this object: <code>dlxExchangeName</code> and <code>dlxRoutingKey</code>. <br>
   * <code>maxRetryCount</code> is 3 by default, but you can set it using <code>
   * setMaxRetryCount(int)</code>
   *
   * @param message AMQP message that caused error
   * @param channel channel for AMQP message
   * @param deliveryTag message delivery tag
   * @return <code>true</code> if error handler works sucessfully, <code>false</code> otherwise
   */
